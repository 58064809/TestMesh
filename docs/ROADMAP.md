...
| P04 | 扩展 UI、APP、性能与安全测试 | 进行中（P04-A、P04-B、P04-C 已完成） | 获批后进入 P04-D；P04 全部验收通过后进入 P05 |
...
- **状态**：进行中。P04-A Playwright、P04-B Android Emulator + Appium 和 P04-C k6 已完成真实闭环验收。P04-C 固定使用宿主 Windows 上的 k6 2.2.0、单脚本、单进程、zero retry，并以 `--summary-export` JSON 作为唯一结构化结果源。详见 `docs/PHASES/P04.md`。P04-D ZAP 未授权，不得实施。
